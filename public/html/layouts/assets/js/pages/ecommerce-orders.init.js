...
function loadCharts() {
    var e;
    (e = getChartColorsArray("line_chart_basic")) && (e = {
        series: [{
            name: "New Orders",
    ...
    (linechartBasicChart = new ApexCharts(document.querySelector("#line_chart_basic"),e)).render())
}
...
var perPage = 10
  ...
  , options = (checkAll && (checkAll.onclick = function() {
    for (var e = document.querySelectorAll('.form-check-all input[type="checkbox"]'), t = document.querySelectorAll('.form-check-all input[type="checkbox"]:checked').length, a = 0; a < e.length; a++)
        ...
        0 < t ? document.getElementById("remove-actions").classList.add("d-none") : document.getElementById("remove-actions").classList.remove("d-none")
}
),
{
    valueNames: ["order_id", "order_date", "delivery_date", "products", "customer", "shop", "payment_method", "amount", "rating", "status"],
    page: perPage,
    pagination: !0,
...
sorttableDropdown && sorttableDropdown.forEach(function(a) {
    a.querySelectorAll(".dropdown-menu .dropdown-item").forEach(function(t) {
...
function isStatus(e) {
    switch (e) {
    case "New":
        return '<span class="badge bg-primary-subtle text-primary">' + e + "</span>";
    case "Pending":
    ...
    case "Shipping":
        return '<span class="badge bg-info-subtle text-info">' + e + "</span>";
    case "Delivered":
...
xhttp.onload = function() {
    ...
    Array.from(e).forEach(function(e) {
        orderList.add({
            order_id: `<a href="apps-ecommerce-order-overview.html" class="fw-medium link-primary">#TBS2500 ${e.id}</a>`,
            order_date: e.order_date,
            delivery_date: e.delivery_date,
            products: e.product,
            customer: e.customer,
            shop: '<a href="#!" class="text-reset"><img src="' + e.shop[0].img + '" alt="" class="avatar-xxs rounded-circle me-1 shop-logo"> <span class="shop-name">' + e.shop[0].name + "</span></a>",
            payment_method: e.pay_method,
            amount: '<span class="fw-medium">' + e.price + "</span>",
            rating: '<h5 class="fs-md fw-medium mb-0">' + e.ratings + "</h5>",
            status: isStatus(e.delivery_status)
        }),
        orderList.sort("order_id", {
            order: "desc"
        })
    }),
...
document.querySelector(".pagination-next").addEventListener("click", function() {
    document.querySelector(".pagination.listjs-pagination") && document.querySelector(".pagination.listjs-pagination").querySelector(".active") && null != document.querySelector(".pagination.listjs-pagination").querySelector(".active").nextElementSibling && document.querySelector(".pagination.listjs-pagination").querySelector(".active").nextElementSibling.children[0].click()
}),
document.querySelector(".pagination-prev").addEventListener("click", function() {
    document.querySelector(".pagination.listjs-pagination") && document.querySelector(".pagination.listjs-pagination").querySelector(".active") && null != document.querySelector(".pagination.listjs-pagination").querySelector(".active").previousSibling && document.querySelector(".pagination.listjs-pagination").querySelector(".active").previousSibling.children[0].click()
}),
document.getElementById("showModal").addEventListener("show.bs.modal", function(e) {
    ...
    document.getElementById("add-btn").innerHTML = "Update") : e.relatedTarget.classList.contains("add-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Add Order",
    document.getElementById("showModal").querySelector(".modal-footer").style.display = "block",
    document.getElementById("add-btn").innerHTML = "Add Order") : (document.getElementById("exampleModalLabel").innerHTML = "List product",
...
var idField = document.getElementById("id-field")
  ...
  , shopNameField = document.getElementById("shopName-input")
  , productsField = document.getElementById("productname-field")
  , orderDateField = document.getElementById("date-field")
...
function ischeckboxcheck() {
    Array.from(document.getElementsByName("chk_child")).forEach(function(a) {
        a.addEventListener("change", function(e) {
            ...
            0 < t ? document.getElementById("remove-actions").classList.remove("d-none") : document.getElementById("remove-actions").classList.add("d-none")
        })
    })
}
function refreshCallbacks() {
    removeBtns && Array.from(removeBtns).forEach(function(e) {
        e.addEventListener("click", function(e) {
            ...
            Array.from(e).forEach(function(e) {
                var e = (new DOMParser).parseFromString(e._values.order_id, "text/html")
                  , t = e.body.firstElementChild;
                e.body.firstElementChild.innerHTML == itemId && document.getElementById("delete-record").addEventListener("click", function() {
                    orderList.remove("order_id", t.outerHTML),
    ...
    editBtns && Array.from(editBtns).forEach(function(e) {
        e.addEventListener("click", function(e) {
            ...
            Array.from(e).forEach(function(e) {
                var t, a = (isid = (new DOMParser).parseFromString(e._values.order_id, "text/html")).body.firstElementChild.innerHTML;
                a == itemId && (editList = !0,
                idField.value = a,
                customerNameField.value = e._values.customer,
                productsField.value = e._values.products,
                paymentField.value = e._values.payment_method,
                a = (new DOMParser).parseFromString(e._values.status, "text/html"),
                deliverStatsField.value = a.body.querySelector(".badge").innerHTML,
                orderDateField.value = e._values.order_date,
                t = (new DOMParser).parseFromString(e._values.amount, "text/html"),
                amountField.value = t.body.querySelector("span").innerHTML,
                productVal && productVal.destroy(),
                (productVal = new Choices(productsField,{
                    searchEnabled: !1
                })).setChoiceByValue(e._values.products),
                t = (new DOMParser).parseFromString(e._values.shop, "text/html"),
                shopNameField.value = t.body.querySelector(".shop-name").innerHTML,
                companyLogoImg.src = t.body.querySelector(".shop-logo").src,
                flatpickr("#date-field", {
                    enableTime: !0,
                    dateFormat: "d M, Y",
                    defaultDate: e._values.order_date
                }),
                paymentVal && paymentVal.destroy(),
                (paymentVal = new Choices(paymentField,{
                    searchEnabled: !1
                })).setChoiceByValue(e._values.payment_method),
                deliverStatsVal && deliverStatsVal.destroy(),
                (deliverStatsVal = new Choices(deliverStatsField,{
                    searchEnabled: !1
                })).setChoiceByValue(a.body.querySelector(".badge").innerHTML),
                document.getElementById("delivery-status-field").value = e._values.delivery_date,
                t = (new DOMParser).parseFromString(e._values.rating, "text/html"),
                document.getElementById("rating-field").value = t.body.querySelector("h5").innerHTML)
            })
        })
    })
}
